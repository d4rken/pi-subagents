...
- Reach pi-ai's `createInitialSystemMessage` and `toToolDeclaration` through a
  ...
  before it runs; the fallback keeps this release loadable on 0.85.x.
- Assemble a child's prompt from its resource loader instead of editing the rendered
  string afterwards. Inheritance exclusions and the child boundary are applied before
  `buildSystemPrompt` runs, so the prompt other extensions record at
